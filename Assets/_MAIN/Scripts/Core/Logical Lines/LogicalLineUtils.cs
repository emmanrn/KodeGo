...
// this script provides shared variables and method access
// to Logical Line classes
namespace DIALOGUE.LogicalLines
{
    public static class LogicalLineUtils
    {
            ...
            public static EncapsulatedData RipEncapsulationData(Conversation conversation, int startingIndex, bool ripHeaderAndEncapsulators = false, int parentStartingIndex = 0)
            {
                ...
                for (int i = startingIndex; i < conversation.Count; i++)
                {
                    ...
                    if (IsEncapsulationEnd(line))
                    {
                        ...
                        // this means that we have found the last one
        ...
        // the condition lines will actually use a Conditions class.
        // But the operators use the Expressions class
        public static class Expressions
        {
            ...
            private const char NEGATION_SYMBOL = '!';
            ...
            public static object CalculateValue(string[] expressionParts)
            {
                ...
                for (int i = 0; i < expressionParts.Length; i++)
                {
                    ...
                    // if this is an operator add to the list of operators but as a string value
                ...
                CalculateValue_DivisionMultiplication(operatorStrings, operands);

                CalculateValue_AdditionSubtraction(operatorStrings, operands);

                return operands[0];

            }

            private static void CalculateValue_DivisionMultiplication(List<string> operatorStrings, List<object> operands)
            {
                for (int i = 0; i < operatorStrings.Count; i++)
                {
                    ...
                    if (operatorString == "*" || operatorString == "/")
                    {
                        ...
                        // we remove the right hand operand because we are updating the left one
            ...
            private static void CalculateValue_AdditionSubtraction(List<string> operatorStrings, List<object> operands)
            {
                for (int i = 0; i < operatorStrings.Count; i++)
                {
                    string operatorString = operatorStrings[i];
            ...
            private static object ExtractValue(string value)
            {
                ...
                if (value.StartsWith(NEGATION_SYMBOL))
                {
                    negate = true;
        ...
        public static class Conditions
        {
            ...
            public static bool EvaluateCondition(string condition)
            {
                ...
                {
                    // basically stripping off the quotes from the strings
                    // this is because if we have a condition like this where
                    //
                    // $str = "My string"
                    // if ($myString == "My string")
                    //
                    // the "My string" that we are comparing will have the quotes in them still
                    // while the $str variable wont have the quotes because it's already been parsed as a string
                    if (parts[i].StartsWith("\"") && parts[i].EndsWith("\""))
                        parts[i] = parts[i].Substring(1, parts[i].Length - 2);
                }

                // if the length of the parts is just 1 then we are sure that that is just a bool variable 
                // e.g if ($isAlive)
                if (parts.Length == 1)
                {
            ...
            private static bool EvaluateExpression(string left, string op, string right)
            {
                ...
                {
                    // safeguarding if somehow we made an invalid condition
                ...
                // then anything we'll assume it's just a string
                switch (op)
                {
