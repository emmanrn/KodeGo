...
public class AudioManager : MonoBehaviour
{
    ...
    public AudioSource PlaySoundEffect(AudioClip clip, AudioMixerGroup mixer = null, float volume = 1f, float pitch = 1, bool loop = false)
    {
    ...
    public void StopTrack(string trackName)
    {
        ...
        {
            if (channel.activeTrack.name.ToLower() == trackName)
            {
                channel.StopTrack();
                return;
            }
